...
> **この repo には2つの使い方があります。**
>
> 1. **プロジェクトの雛形として丸ごと使う**（Dev Container・MCP 設定ごとクローン）— このドキュメントを Step 1 から順に進める。
> 2. **`dev` plugin だけを既存プロジェクトに導入する** — [README](README.md) の「インストール」または下の Step 3.8 だけでよい（Step 1・2 の Dev Container 準備は不要）。
>
> どちらでも `dev` plugin（エージェント・コマンド・スキル）は同じように使えます。
...
**PRD（product-requirements）のみ対話式で確認しながら作成し、残り5つの永続ドキュメントは承認後に自動生成される。** 結果として `docs/` 配下に次の6つが揃う。
