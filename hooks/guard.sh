...
#   - レビュー完走マーカーの無い `gh pr create` を block する（5 参照。セルフテスト: hooks/guard-test.sh）。
...
# cross-repo / worktree 運用（シェルは repoA、コマンドは `cd repoB && git ...` / `cd repoB && gh pr create`）で
# ブランチ判定・レビュー証跡を「実際に操作される repo」に対して行うため（1/4/5 の cross-repo 盲点対策）。
# 安全側の設計（すり抜け防止 > 追従性）:
...
# すり替わり、ブランチ判定・レビュー証跡がそちらを基準に行われてしまう）。
...
#     フレーズを含むだけのコミットメッセージ等でも発火する（5) のレビュー証跡ゲートと同種の既知の限界）。
...
# 5) レビュー証跡ゲート: レビュー完走マーカーの無い `gh pr create` を block（Issue #19 決定4）
#    マーカーは /ship がレビュー clean 到達時に git ディレクトリ配下(--absolute-git-dir)の
#    claude/review-passed へ作成する（worktree では .git/worktrees/<name>/claude/... になる）
#    （branch+sha 束縛。レビュー後にコミットが積まれたら無効）。人間の手動手順は README 参照。
#    受動的なレビュー省略を止めるゲート。既知の限界:
#      - マーカー自体は偽造検知不能（偽造禁止はプロンプト層の抑止）
#      - repo 外からの `gh pr create -R owner/repo` や `gh api` 直叩きはフェイルオープン
#      - コマンド文字列にフレーズを含むだけ（コミットメッセージ等）でも発火しうる（フェイルクローズ
#        側の誤検知）。ただし --body/-m 等 allowlist 済みフラグのヒアドキュメント本文（cmd_scan で
#        除去済み）内のフレーズはここでは発火しない（Issue #61 の4回目レビューで判明・修正）。
if printf '%s' "$cmd_scan" | grep -Eq '(^|[^[:alnum:]_])gh([[:space:]]+-[^[:space:]]+)*[[:space:]]+pr[[:space:]]+create([[:space:];&|<>]|$)'; then
  # 5a) HEAD/ブランチを動かすコマンドとの連鎖は、フックが実行"前"の状態しか見られず
  #     branch+sha 束縛を素通りさせるため、マーカーの有無に関わらず block（1b と同型の穴塞ぎ）。
  #     `git checkout <file>`（ファイル復元）連鎖も block される誤検知はセクション4と同様に許容。
  if has_git_subcmd '(commit|merge|rebase|cherry-pick|checkout|switch|pull|reset)'; then
    block "HEAD/ブランチを動かすコマンドと PR 作成の連鎖を検出。実行後の状態ではレビュー証跡を検証できません。コマンドを分けて、変更後に再レビューしてください。"
  fi
  git_dir=$(git -C "$eff_cwd" rev-parse --absolute-git-dir 2>/dev/null || echo "")
  if [ -n "$git_dir" ]; then
    marker="$git_dir/claude/review-passed"
    cur="${BRANCH_OVERRIDE:-$(git -C "$eff_cwd" rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")}"
    head_sha=$(git -C "$eff_cwd" rev-parse HEAD 2>/dev/null || echo "")
    if [ ! -f "$marker" ]; then
      block "レビュー未完走: gh pr create の前にレビューを完走させてください。/ship を使うか、手動レビュー後に README「レビュー証跡ゲート」の手順でマーカーを作成すること。"
    fi
    m_branch=$(sed -n 's/^branch=//p' "$marker" | head -1)
    m_sha=$(sed -n 's/^sha=//p' "$marker" | head -1)
    if [ "$m_branch" != "$cur" ]; then
      block "レビュー証跡のブランチ不一致: マーカーは '${m_branch}'、現在は '${cur}'。このブランチでレビューを完走し直してください。"
    fi
    if [ "$m_sha" != "$head_sha" ]; then
      block "レビュー後にコミットが追加されています（レビュー時 ${m_sha} ≠ HEAD）。再レビューしてマーカーを更新してください。"
    fi
  fi
fi
