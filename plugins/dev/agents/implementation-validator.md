...
- **PASS** = リポに存在する機械検証（test / lint / typecheck 等、**あるものだけ**）が全て成功 **かつ** [必須] 重大な問題が 0 件
- **FAIL** = それ以外（不足している点を「検出された問題」に列挙する）
- 機械検証が存在しないリポでは「機械検証: 該当なし」と明記し、重大な問題の有無のみで判定する
- 1〜5点スコアと総合スコアは**参考情報**であり、合否の根拠にしない（特定の開発手法にも依存しない）
...
> 以下の npm コマンドは**例**。リポに存在する検証（package.json・`docs/development-guidelines.md`・Makefile 等から特定）だけを実行し、存在しないものは「該当なし」として合否判定に含めない（ADR 0008）。
