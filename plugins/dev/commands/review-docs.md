...
Agent toolを使用してdoc-reviewerサブエージェントを起動してください:
...
サブエージェントが作成したレビューレポートから、優先度「高」の指摘を最大3件抽出し、ユーザーに報告します。
...
- doc-reviewer のレビューレポートが生成されている
- 優先度「高」の指摘（最大3件）がユーザーに提示されている
