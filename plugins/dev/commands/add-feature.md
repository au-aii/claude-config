---
name: add-feature
description: 新機能を既存パターンに従って、完全に無停止で実装する。使う場面：実装パターンが既存で影響範囲が読める定型の機能追加。使わない場面：影響範囲不明・複数ファイル横断（/asdd）・人間の承認ゲートを挟みたい（/design）・計画ファイルだけ欲しい（/setup-steering）
---
...
**引数:** Issue番号 機能名 (例: `/add-feature 42 ユーザープロフィール編集`)

- Issue番号は省略可。省略時はブランチ作成をスキップする。
...
- 第1引数が数値の場合: Issue番号として扱い、残りを機能名とする
- 第1引数が数値でない場合: Issue番号なし、引数全体を機能名とする

2. 現在のタスクコンテキストを確立する:

- Issue番号: `[解析したIssue番号、なければ "-"]`
- 機能名: `[解析した機能名]`
- 日付: `[現在の日付をYYYYMMDD形式で取得]`
- ステアリングディレクトリパス: `.steering/[日付]-[機能名]/`

3. Issue番号がある場合、以下のコマンドで紐づいたブランチを作成してチェックアウトする:

```bash
gh issue develop [Issue番号] --checkout
```

- このコマンドにより、GitHub上でIssueとブランチが自動リンクされる
- Issue番号がない場合はこの手順をスキップする

4. 上記ステアリングディレクトリを作成する。
5. 以下の3つの空ファイルを作成する:
...
- **ルールA: タスクが大きすぎる場合**
  ...
  - （基準: 実装に新規ファイル3つ以上、または半日以上かかると見積もられる場合）
...
1. プロジェクトで定義されているテスト・リント・型チェックのコマンドを確認し、順番に実行する。

- `package.json` がある場合: `npm test`, `npm run lint`, `npm run typecheck` など
- その他の言語・ツールの場合: `docs/development-guidelines.md` に記載されたコマンドを参照
- コマンドが未定義の場合: このステップをスキップし、その旨を振り返りに記録する
...
2. 今回の変更がプロジェクトの基本設計やアーキテクチャに影響を与えるか判断する。
   判定基準: 新規ディレクトリ作成・外部依存の追加・既存 API シグネチャの変更のいずれかに該当すれば影響あり。
...
- ステップ6: `implementation-validator`サブエージェントの合否判定が **PASS** である（FAIL の場合は指摘を修正して再検証する）。
...
実装が完了したら `/ship` を実行する。push→レビュー→PR作成→**マージまで**一気通貫で行う（`/ship` はフルオート＝マージ承認込みの明示コマンド）。

マージ前に一度止めてレビューだけ確認したい場合は `/ship` を使わず、`/commit-push` でコミット・プッシュ → fresh subagent でレビューを完走 → README「レビュー証跡ゲート」の手順でマーカーを作成 → `gh pr create` で PR を作成して**そこで停止**する（既定のフロー。マーカー無しの `gh pr create` は `hooks/guard.sh` が物理ブロックする点に注意。マージは別途ゴーサインを待つ）。
